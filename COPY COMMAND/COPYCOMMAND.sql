...
alter file format  MDB.FFMT.csvf set error_on_column_count_mismatch=false;
LIST @mdb.e_stg.estg3;
COPY into MDB.public.Movies from @MDB.E_STG.estg3 
file_format = (format_name = MDB.FFMT.csvf )
files = ('streaming_data.csv');

CREATE TABLE MDB.PUBLIC.LOANS( Loan_ID varchar(200),
                    ...
                    past_due_days varchar(200),
                    age number(4),
                    education varchar(200),
                    Gender varchar(200));
--drop table LOANS;

describe table MDB.PUBLIC.LOANS;
COPY into MDB.public.Loans from @MDB.E_STG.estg3 
file_format = (format_name = MDB.FFMT.csvf )
files = ('loan_payments_data.csv')
ON_ERROR = 'CONTINUE';
